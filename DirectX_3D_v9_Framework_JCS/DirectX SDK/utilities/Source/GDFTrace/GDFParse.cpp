...
//--------------------------------------------------------------------------------------
HRESULT CGDFParse::LoadXML( WCHAR* strGDFPath )
{
    SAFE_RELEASE( m_pRootNode );
...
{   
    ...
    HMODULE hGDFDll = LoadLibraryEx( strGDFBinPath, NULL, LOAD_LIBRARY_AS_DATAFILE);
    SetErrorMode( old );
...
static void MakeExePathXSD( WCHAR *result, size_t rsize, WCHAR *fname )
{
    ...
    if ( GetModuleFileNameEx( GetCurrentProcess(), NULL, exepath, MAX_PATH ) )
    {
...
//--------------------------------------------------------------------------------------
HRESULT CGDFParse::ValidateGDF( WCHAR* strGDFPath, WCHAR* strReason, int cchReason )
{
    bool bValidateSuccess = false;
   ...
   if( SUCCEEDED(hr) ) 
   {
    ...
    if( !bLoadSuccess )
    {
        StringCchCopy( strReason, cchReason, L"Could not load GDF.  Verify GDF XML is valid" );
        return S_OK;
    }
...
//--------------------------------------------------------------------------------------
HRESULT CGDFParse::ExtractGDFThumbnail( WCHAR* strGDFBinPath, D3DX10_IMAGE_INFO* info, WORD wLanguage )
{
        ...
        { 
            ...
            { 
                ...
                { 
                    ...
                    {
                        hr = D3DX10GetImageInfoFromMemory( pResourceBuffer, dwGDFThumbnailSize, NULL, info, NULL );
                    }
                } 
            }
        }
...
//--------------------------------------------------------------------------------------
unsigned int GetIntBigEndian(__in_ecount(4) const unsigned char* pData)
{
    return ((int)pData[0]<<24) + ((int)pData[1]<<16) + ((int)pData[2]<<8) + (int)pData[3];
}

//--------------------------------------------------------------------------------------
unsigned int GetIntLittleEndian(__in_ecount(4) const unsigned char* pData)
{
    return ((int)pData[3]<<24) + ((int)pData[2]<<16) + ((int)pData[1]<<8) + (int)pData[0];
}

//--------------------------------------------------------------------------------------
unsigned int GetShortLittleEndian(__in_ecount(2) const unsigned char* pData)
{
    return ((int)pData[1]<<8) + (int)pData[0];
}

//--------------------------------------------------------------------------------------
BOOL IsPNG(__in_ecount(8) const unsigned char* pData)
{
...
//--------------------------------------------------------------------------------------
bool GetWidthAndHeightPNG(__in_ecount(24) const unsigned char* pData, __out unsigned int* pwidth, __out unsigned int* pheight)
{
    // Double check
    if (!IsPNG(pData))
    {
    ...
    unsigned char   IHDR[] = {'I', 'H', 'D', 'R'};
    for (int i = 0; i < sizeof(IHDR)/ sizeof(unsigned char); i++) 
    {
        if (IHDR[i] != pData[12+i])
        {
            return false;
        }
    }

    // Ok, we've got the image header, now extract the width and height
    *pwidth = GetIntBigEndian(&pData[16]);
    *pheight = GetIntBigEndian(&pData[20]);
            ...
            if( hgResource ) 
            {
                ...
                for(int i = 0; i < lpIcon->idCount; i++ )
                {
                    ...
                    if (lpPNG)
                    {
                        ...
                        if (IsPNG(lpPNG))
                        {
                            GetWidthAndHeightPNG(lpPNG, &uWidth, &uHight);
                            D3DX10_IMAGE_INFO info;
                            dwGDFIconSize = SizeofResource( hGDFDll, hrsrc );
                            if( dwGDFIconSize )
                            {
                                hr = D3DX10GetImageInfoFromMemory( lpPNG, dwGDFIconSize, NULL, &info, NULL );
                            }
                            switch(info.Format)
                            {
                            ...
                            wprintf( L"\tIcon: %dx%d %dbits PNG\n", uWidth, uHight,wBitCount);
                        }
                        else 
                        {
                            LPICONIMAGE lpBMP = (LPICONIMAGE)lpPNG;
                            uWidth = lpBMP->icHeader.biWidth;
                            uHight = lpBMP->icHeader.biHeight/2;
                            wprintf( L"\tIcon: %dx%d %dbits BMP\n", uWidth, uHight,wBitCount);
                        }

                        for (int res=0; res<4; res++)
                        {
                            if(iconResolution[res] == uWidth && iconResolution[res] == uHight)
                            {
                                if (wBitCount == 8)
                                {
...
//--------------------------------------------------------------------------------------
BOOL CGDFParse::IsIconPresent( WCHAR* strGDFBinPath )
{
    BOOL res = FALSE;

    UINT old = SetErrorMode( SEM_FAILCRITICALERRORS | SEM_NOOPENFILEERRORBOX );
    HICON hicon = ExtractIcon( NULL, strGDFBinPath, 0 );
    SetErrorMode( old );
...
HRESULT CGDFParse::IsV2GDF (BOOL* pfV2GDF)
{
    ...
    *pfV2GDF = FALSE;
    if (m_pRootNode != NULL)
    {
        ...
        hr = m_pRootNode->selectSingleNode(L"//GameDefinitionFile/GameDefinition/ExtendedProperties/GameTasks/Play/Primary", &pPrimaryPlayTasksNode);
        if (S_OK == hr)
        {
            *pfV2GDF = TRUE;
        }
